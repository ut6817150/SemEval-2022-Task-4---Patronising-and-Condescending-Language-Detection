...
│
├── Exercise_2.ipynb                         # Exploratory Data Analysis
...
A CUDA-capable GPU is strongly recommended for Exercise 4. Exercise 5 runs on CPU or MPS (Apple Silicon).
...
Open and run `Exercise_2.ipynb`. This notebook covers class distribution, token length analysis, lexical analysis, semantic exploration, and noise identification. No GPU required.

### Step 2 — Training

The `Models/` directory already contains saved model weights for both the baseline and all four ensemble members. `Exercise_4.ipynb` will load these automatically without retraining.

To retrain from scratch, delete the contents of `Models/BaselineModel/` and `Models/BestModel/` and run `Exercise_4.ipynb`. Each model will be retrained and saved back to the same directories.
...
Open and run `Exercise_5.ipynb`. This notebook is **fully standalone** — it loads all models and the decision threshold directly from disk with no dependency on Exercise 4 runtime variables. It produces:
